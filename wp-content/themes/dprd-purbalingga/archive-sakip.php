...
// Ambil semua pos CPT SAKIP terurut kronologis/pos
$sakip_query = new WP_Query([
    'post_type'      => 'sakip',
    'posts_per_page' => -1,
    'orderby'        => 'date',
    'order'          => 'ASC',
...
<main id="primary" class="w-full bg-main min-h-screen pt-10 pb-24">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-16">
        ...
        <div class="w-full flex flex-col border-t border-primary/40 mt-12">
            <?php if ($sakip_query->have_posts()) : 
                $count = 0;
                while ($sakip_query->have_posts()) : $sakip_query->the_post();
                    $count++;
                    $post_id     = get_the_ID();
                    $title       = get_the_title();
                    ...
                    // Fallback jika ada meta file_url lama
                    if (empty($documents)) {
                        $old_file = get_post_meta($post_id, 'file_url', true);
                        if (!empty($old_file)) {
                            $documents = [['title' => $title, 'url' => $old_file]];
                        }
                    }
                    
                    // Default open item pertama
                    $is_open = ($count === 1);
                    ?>
                ...
                <p class="font-sans text-body-secondary py-8 italic">Belum ada dokumen SAKIP yang terdaftar.</p>
            <?php endif; ?>
        </div>
